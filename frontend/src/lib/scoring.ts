...
export function calcPreview(params: {
  ...
  const { ingresos, valorInmueble, montoSolicitado, subtipo, plazoMeses } = params
  ...
  const cuotaMaxima = ingresos * MAX_DEUDA_RATIO
  const ltv = montoSolicitado / valorInmueble
  const excede = cuotaTotal > cuotaMaxima

  return { cuotaTotal, cuotaMaxima, ltv, excede, cuotaSinSeguro, seguroVida, seguroIncendio }
}
